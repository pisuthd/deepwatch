...
interface RawMarket {
  ...
  endDate?: string;
  /**
   * For "Up or Down" intraday markets, the start time of the 1-hour
   * candle that the market covers. We use this to fetch the "Price To
   * Beat" (the open price of that candle) from Binance.
   */
 ...
 * ("This market will resolve to 'Up' if …"). Rules text is useless as
 * a per-row label on the card. Heuristic: a clean label is short
 * (< 60 chars) and doesn't read like legalese. Anything longer or
 * containing "resolve" / "resolution" / "this market" is treated as
 * rules text and replaced with null so the card falls back to the
 * generated label.
 */
 ...
 * Returns the open price of the matching 1-hour candle.
 *
 * Returns null on any failure (network, missing candle, etc.) — the
 * card then falls back to a plain "Up" label.
 */
async function fetchPriceToBeatUsd(
  ...
  // Round down to the start of the 1-hour candle that contains eventStartTime.
...
export async function fetchPolymarketMarkets(
  ...
  for (const e of events) {
    ...
    for (const m of e.markets ?? []) {
      ...
      // Fetch the "Price To Beat" for "Up or Down" markets that have an
      // eventStartTime. Only relevant for the binary intraday shape
      // (e.g. "Bitcoin Up or Down - June 16, 9AM ET") where there's
      // no strike — the card uses it as part of the row label.
      ...
      for (let i = 0; i < outcomes.length; i++) {
        ...
        out.push({
          ...
          question: m.question ?? "",
          // Filter the API description: clean outcome labels (e.g.
          // "$55,500 or above") pass through; rules text falls back
          // to null so the card uses its generated label.
