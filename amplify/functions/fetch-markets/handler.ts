...
/**
 * Scheduled handler. Runs every 15 minutes via EventBridge.
 * Fetches the three platforms in parallel; a single platform failure does
 * not abort the rest of the batch. Each platform's rows are upserted
 * (create only — we treat every fetch as a fresh snapshot).
 */
...
> = async (event) => {
  ...
  const stats = {
    polymarket: { written: 0, failed: 0, error: null as string | null, fetched: 0 },
    kalshi: { written: 0, failed: 0, error: null as string | null, fetched: 0 },
    deepbook: { written: 0, failed: 0, error: null as string | null, fetched: 0 },
  };
  ...
  async function writeBatch<T extends Record<string, unknown>>(
    ...
    stat: { written: number; failed: number; error: string | null; fetched: number },
  ) {
