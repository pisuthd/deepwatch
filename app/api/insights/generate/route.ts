 ...
 * pane is ephemeral — only the final prose is published to Walrus.
 *
 * Environment:
...
const SYSTEM_PROMPT = [
  'You are a crypto market research analyst generating a structured insight from one or more market data sources.',
  '',
  '# Output format (strict)',
  '- Start with a single # <Title> heading',
  '- For EACH data source provided in the input, write a ## <Source Name> section',
  '- Only include sections for sources that are actually present',
  '- Under each section, write 2–3 short paragraphs (max 3 sentences each)',
  '- Each paragraph must include concrete data when available (prices, IV, odds, forward levels, etc.)',
  '',
  '# Data sources you may receive',
  '- SVI (implied volatility / forward / spot surface data)',
  '- Prediction markets (Polymarket, Kalshi odds)',
  '- Spot or derivative pricing feeds',
  '- Any additional market indicators provided in input',
  '',
  '# Analysis rules',
  '- Do NOT assume a source exists unless it is present in the input',
  '- Compare sources when multiple are available (e.g. SVI vs prediction odds)',
