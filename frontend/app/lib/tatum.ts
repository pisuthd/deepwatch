...
/**
 * Filename shape:
 *   - plaintext blob: `analysis-batch-<batchId>-<timestampMs>.json`
 *   - encrypted blob: `analysis-batch-<batchId>-enc-<timestampMs>.json`
 *
 ...
 *
 * The two shapes share a `<batchId>` stem so callers can pair the
 * plaintext and encrypted blobs without reading the body. The `-enc`
 * marker is the discriminator; `parseBatchFilename` returns it as
 * `encrypted: boolean` so the recent-batches panel can hide the
 * sealed-slice rows from the default view (they're not readable
 * without a Subscription NFT anyway).
 *
 * Examples:
 *   analysis-batch-a1b2c3d4-1749056400000.json
 *   analysis-batch-a1b2c3d4-enc-1749056400000.json
 *   analysis-batch-e5f6g7h8-1749052800000.json
 */
const BATCH_FILENAME_RE = /^analysis-batch-([a-z0-9]{8})(-enc)?-(\d+)\.json$/i;

export interface ParsedBatchFilename {
  batchId: string;
  timestamp: number;
  /** True if the filename carries the `-enc` marker (Seal-encrypted slice). */
  encrypted: boolean;
}

/**
 * Parse a Walrus upload filename into `(batchId, timestamp,
 * encrypted)`. Returns `null` if the filename doesn't match the
 * batch convention (so the recent-batches list can filter to just
 * the AI batch uploads and skip any stray files the API key has
 * produced).
 */
 ...
 *
 * Pass `options.encrypted: true` to produce the `-enc` variant used
 * for the Seal-encrypted slice. Without `options` the filename is the
 * plaintext (free slice) variant.
 */
export function batchFilename(
  ...
  const tag = options.encrypted ? '-enc' : '';
  return `analysis-batch-${batchId}${tag}-${timestampMs}.json`;
}
