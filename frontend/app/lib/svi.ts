 ...
 * probability calculation. Pure functions, no React/Next dependencies.
 *
 * Ported from /lib/markets/svi.ts so the frontend can run without
 * depending on the root app's lib/. Identical math, identical scales:
...
export function impliedProbUpForStrike(
  ...
  // Default SVI (matches the fallback in /lib/markets/svi.ts).
